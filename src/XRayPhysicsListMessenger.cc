...
XRayPhysicsListMessenger::XRayPhysicsListMessenger(XRayPhysicsList* pPhys)
:pPhysicsList(pPhys)
{
...
XRayPhysicsListMessenger::~XRayPhysicsListMessenger()
{
  delete pListCmd;
  delete gammaCutCmd;
  delete electCutCmd;
...
void XRayPhysicsListMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{ 
  if( command == pListCmd )
